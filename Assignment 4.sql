...
-- Q.2 Choose a table in sakila and describe how you would determine whether it is i in 2NF.If it violates 2NF explain steps to normalize it.
--    Solution -
...
-- Q.3 Identify the sakila that violates 3NF. 
-- Describe the transitive dependencies present and outline the unnormalized part upto at least 2NF.
-- Solution
...
-- by following these steps, we've normalized the payment table up to the Second Normal Form (2NF) 
-- by removing the partial dependencies between payment attributes. Now, the payment table conforms to 2NF.

-- Q.5 Write a query using cte to retrieve the distinct list of actor names and number of films they have acted in from the actor and film_actor table
with actorCount as( 
...
-- Q.6 Use recursive cte to generate a hierachical list of catagories and their subcatagories from the catagory table
...
-- Q.8 Write a query using cte to find the total revenue generated by each customer (sum of payments) from the customer and payment tables.
...
-- Q.11 Write a query using a cte to find the total number of rentals made each month, considering the rental_date from rental table.
with total_rentals as (
select
...
-- Q.12 Use cte to pivot data from the payment table to display the total payments made by each customer in separate column for different payment methods.
...
-- Q.14 Implement a recursive cte to find all employees in the staff table who report to specific manager considering the reports_to column
